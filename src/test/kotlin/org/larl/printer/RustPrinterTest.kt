...
internal class RustPrinterTest : BaseMockingTest {
    private lateinit var printer: RustPrinter

    @BeforeEach
    fun setUp() {
        printer = mock(RustPrinter::class.java)
        ...
        doCallRealMethod().`when`(printer).trimEmptyLines(anyString())
    }

    @Test
    fun `should print the expected module`() {
        val actual = printer.handleModule(complexModuleFixture)
        val expected = this::class.java.getResource("/org/larl/printer/RustPrinterTest/expectedModule.txt").readText()
